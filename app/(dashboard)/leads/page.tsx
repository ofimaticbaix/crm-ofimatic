...
export default function LeadsPage() {
  ...
  const handleSetStage = async (newStage: string) => {
    ...
    setStageSaving(true)
    await updateCompany(selectedLead.id, { account_type: (newStage || undefined) as any })
    const updated = { ...selectedLead, account_type: newStage || null }
  ...
  const handleSetStatus = async (newStatus: string) => {
    ...
    setSelectedLead({ ...selectedLead, custom_fields: cf })
  ...
  const sorted = [...filtered].sort((a: any, b: any) => {
    ...
    if (sortField === 'name') { valA = a.name || ''; valB = b.name || '' }
      ...
      const tagA = a.custom_fields?.lead_tag || ''
      const tagB = b.custom_fields?.lead_tag || ''
    ...
    <div className="space-y-5 bg-[#0a1628]/80 backdrop-blur-sm rounded-2xl p-5">
      ...
      {sorted.length > 0 && (
        <p className="text-[10px] text-gray-500 text-center">
          Mostrando {sorted.length} de {leads.length} clientes potenciales
        </p>
      )}
      ...
      {mounted && selectedLead && createPortal(
        <div
          ...
          onClick={() => { setSelectedLead(null); setIsEditing(false); setEditForm(null) }}
        >
