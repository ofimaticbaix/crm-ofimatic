...
import { getClosedCompanies, getContactsByCompany, getDealsByCompany, getDaysSinceLastActivity } from '@/lib/mock-data'
...
export default function ClientesCerradosPage() {
  ...
  const getCompanyData = (companyId: string) => {
    ...
    return { contacts, totalValue, daysSinceContact }
  }
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl md:text-3xl font-bold text-white flex items-center gap-3">
          <Lock className="h-7 w-7 text-gray-400" /> Clientes Cerrados
        </h1>
        <p className="text-xs md:text-sm text-white mt-1">{closedCompanies.length} clientes cerrados</p>
      </div>
      ...
      {closedCompanies.length === 0 && (
        <p className="text-sm text-gray-400 text-center py-8">No hay clientes cerrados</p>
      )}
    </div>
  )
}
