...
import {
  ...
  AlertTriangle, Clock, BarChart3, ArrowUpRight, ArrowDownRight
} from 'lucide-react'
import {
  mockDeals, mockContacts, mockTasks, mockActivities, mockCompanies,
  getActiveCompanies, getClosedCompanies, getOverdueCompanies,
  calculatePipelineMetrics, stages
} from '@/lib/mock-data'
import { formatCurrency } from '@/lib/utils'

export default function MetricsPage() {
  const pipeline = calculatePipelineMetrics()
  ...
  const totalRevenue = mockDeals.reduce((sum, d) => sum + d.value, 0)
  const avgDealSize = totalRevenue / mockDeals.length

  const contactsByLifecycle = {
    customer: mockContacts.filter(c => c.lifecycle === 'customer').length,
    prospect: mockContacts.filter(c => c.lifecycle === 'prospect').length,
    lead: mockContacts.filter(c => c.lifecycle === 'lead').length,
  }

  const dealsByStage = stages.map(s => ({
    ...s,
    count: mockDeals.filter(d => d.stage === s.id).length,
    value: mockDeals.filter(d => d.stage === s.id).reduce((sum, d) => sum + d.value, 0),
  }))
  ...
  const taskTypes = {
    tarea: mockTasks.filter(t => t.type === 'tarea').length,
    llamada: mockTasks.filter(t => t.type === 'llamada').length,
    visita: mockTasks.filter(t => t.type === 'visita').length,
    reunion: mockTasks.filter(t => t.type === 'reunion').length,
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-xs md:text-sm text-white mt-1">Vista general de rendimiento del CRM</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        <Card className="border-l-4 border-l-blue-500 hover:shadow-xl transition-shadow">
          <CardContent className="p-4 md:p-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <Badge className="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 text-[10px] rounded-full gap-1">
        ...
        <Card className="border-l-4 border-l-purple-500 hover:shadow-xl transition-shadow">
          <CardContent className="p-4 md:p-6">
            ...
            <p className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">{formatCurrency(avgDealSize)}</p>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between p-3 rounded-xl bg-green-50/50 dark:bg-green-900/10 border border-green-200 dark:border-green-800/30">
              ...
              <span className="text-lg font-bold text-green-600 dark:text-green-400">{activeCompanies.length - overdueCompanies.length}</span>
            </div>
            <div className="flex items-center justify-between p-3 rounded-xl bg-amber-50/50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/30">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <span className="text-[10px] text-gray-400 ml-2">+7 días sin contacto</span>
                </div>
              </div>
              <span className="text-lg font-bold text-amber-600 dark:text-amber-400">{overdueCompanies.length}</span>
            </div>
            <div className="flex items-center justify-between p-3 rounded-xl bg-gray-50/50 dark:bg-gray-800/30 border border-gray-200 dark:border-gray-700/30">
              <div className="flex items-center gap-3">
                <div className="w-3 h-3 rounded-full bg-gray-400" />
                <span className="text-sm font-medium text-gray-900 dark:text-white">Cerrados</span>
              </div>
              <span className="text-lg font-bold text-gray-500 dark:text-gray-400">{closedCompanies.length}</span>
            </div>
            <div className="pt-2 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between">
              <span className="text-sm font-medium text-gray-500 dark:text-gray-400">Total empresas</span>
              <span className="text-lg font-bold text-gray-900 dark:text-white">{mockCompanies.length}</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between p-3 rounded-xl bg-green-50/50 dark:bg-green-900/10 border border-green-200 dark:border-green-800/30">
              ...
              <span className="text-lg font-bold text-green-600 dark:text-green-400">{contactsByLifecycle.customer}</span>
            </div>
            <div className="flex items-center justify-between p-3 rounded-xl bg-blue-50/50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800/30">
              ...
              <span className="text-lg font-bold text-blue-600 dark:text-blue-400">{contactsByLifecycle.prospect}</span>
            </div>
            <div className="flex items-center justify-between p-3 rounded-xl bg-yellow-50/50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-800/30">
              ...
              <span className="text-lg font-bold text-yellow-600 dark:text-yellow-400">{contactsByLifecycle.lead}</span>
            </div>
            <div className="pt-2 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between">
              <span className="text-sm font-medium text-gray-500 dark:text-gray-400">Total contactos</span>
              <span className="text-lg font-bold text-gray-900 dark:text-white">{mockContacts.length}</span>
            </div>
          </CardContent>
        </Card>
      </div>
          ...
          <CardContent className="space-y-3">
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="text-center p-3 rounded-xl bg-amber-50/50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800/30">
                <p className="text-2xl font-bold text-amber-600 dark:text-amber-400">{highPriorityTasks.length}</p>
            ...
            <div className="pt-2 border-t border-gray-200 dark:border-gray-700 space-y-2">
              ...
              {Object.entries(taskTypes).map(([type, count]) => (
                <div key={type} className="flex items-center justify-between text-sm">
                  <span className="text-gray-600 dark:text-gray-300 capitalize">{type === 'reunion' ? 'Reunión' : type.charAt(0).toUpperCase() + type.slice(1)}</span>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="text-center p-4 rounded-xl bg-purple-50/50 dark:bg-purple-900/20 border border-purple-100 dark:border-purple-800/30">
              <p className="text-3xl font-bold text-purple-600 dark:text-purple-400">{mockActivities.length}</p>
            ...
            <div className="pt-2 border-t border-gray-200 dark:border-gray-700 space-y-2">
              ...
              {[
                ...
                { key: 'visit', label: 'Visitas', color: 'text-orange-500' },
                { key: 'note', label: 'Notas', color: 'text-yellow-500' },
              ].map(({ key, label, color }) => (
                <div key={key} className="flex items-center justify-between text-sm">
                  <span className="text-gray-600 dark:text-gray-300">{label}</span>
                  <span className={`font-bold ${color}`}>{activityTypes[key as keyof typeof activityTypes]}</span>
                </div>
              ))}
            </div>
          </CardContent>
