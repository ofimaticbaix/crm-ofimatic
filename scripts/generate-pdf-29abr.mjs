...
;(async () => {
  const pdf = await mdToPdf(
    { path: path.join(root, 'NOVEDADES-FEEDBACK-29-ABR.md') },
    {
      dest: path.join(root, 'NOVEDADES-FEEDBACK-29-ABR.pdf'),
      css,
      pdf_options: {
        format: 'A4',
        ...
        footerTemplate: `<div style="width: 100%; text-align: center; font-size: 9px; color: #9ca3af; padding: 0 15mm;">
          <span>CRM Ofimatic Baix · Respuesta a feedback V.5 · 29 Abr 2026</span>
  ...
  if (pdf) {
    console.log(`PDF generado: ${path.join(root, 'NOVEDADES-FEEDBACK-29-ABR.pdf')}`)
  } else {
