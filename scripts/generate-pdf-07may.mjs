...
;(async () => {
  const pdf = await mdToPdf(
    { path: path.join(root, 'NOVEDADES-FEEDBACK-07-MAY.md') },
    {
      dest: path.join(root, 'NOVEDADES-FEEDBACK-07-MAY.pdf'),
      css,
      pdf_options: {
        format: 'A4',
        ...
        footerTemplate: `<div style="width: 100%; text-align: center; font-size: 9px; color: #9ca3af; padding: 0 15mm;">
          <span>CRM Ofimatic Baix · Respuesta a feedback V.6 · 7 May 2026</span>
  ...
  if (pdf) {
    console.log(`PDF generado: ${path.join(root, 'NOVEDADES-FEEDBACK-07-MAY.pdf')}`)
  } else {
