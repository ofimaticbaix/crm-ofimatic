-- ============================================================
-- Setup for EXISTING users who signed up BEFORE the trigger existed
-- ============================================================
-- Run this AFTER 001_complete_saas_setup.sql
-- This will create workspace + membership + pipeline for any user
-- who doesn't have one yet.
-- ============================================================

DO $$
DECLARE
  usr RECORD;
  new_workspace_id UUID;
  new_pipeline_id UUID;
  user_name TEXT;
BEGIN
  -- Find all auth users who DON'T have a membership yet
  FOR usr IN
    SELECT au.id, au.email, au.raw_user_meta_data
    FROM auth.users au
    LEFT JOIN public.memberships m ON m.user_id = au.id
    WHERE m.id IS NULL
  LOOP
    user_name := COALESCE(
      usr.raw_user_meta_data->>'full_name',
      split_part(usr.email, '@', 1)
    );

    -- 1. Create user profile (if not exists)
    INSERT INTO public.users (id, full_name)
    VALUES (usr.id, user_name)
    ON CONFLICT (id) DO NOTHING;

    -- 2. Create workspace with 14-day trial
    INSERT INTO public.workspaces (name, slug, plan_id, subscription_status, subscription_tier, trial_ends_at)
    VALUES (
      user_name || '''s Workspace',
      lower(replace(replace(user_name, ' ', '-'), '''', '')) || '-' || substr(usr.id::text, 1, 8),
      'starter',
      'trialing',
      'starter',
      now() + interval '14 days'
    )
    RETURNING id INTO new_workspace_id;

    -- 3. Create membership
    INSERT INTO public.memberships (user_id, workspace_id, role)
    VALUES (usr.id, new_workspace_id, 'owner');

    -- 4. Create default pipeline
    INSERT INTO public.pipelines (workspace_id, name, is_default)
    VALUES (new_workspace_id, 'Pipeline Principal', true)
    RETURNING id INTO new_pipeline_id;

    -- 5. Create stages
    INSERT INTO public.stages (pipeline_id, name, description, probability, position, is_closed_won, is_closed_lost) VALUES
      (new_pipeline_id, 'Prospecto',     'Oportunidad identificada',           10, 0, false, false),
      (new_pipeline_id, 'Contactado',    'Primer contacto realizado',          20, 1, false, false),
      (new_pipeline_id, 'Cualificado',   'Necesidades confirmadas',            40, 2, false, false),
      (new_pipeline_id, 'Propuesta',     'Propuesta enviada al cliente',       60, 3, false, false),
      (new_pipeline_id, 'Negociacion',   'En proceso de negociacion',          80, 4, false, false),
      (new_pipeline_id, 'Ganado',        'Deal cerrado con exito',            100, 5, true,  false),
      (new_pipeline_id, 'Perdido',       'Deal no conseguido',                  0, 6, false, true);

    RAISE NOTICE 'Created workspace for user: % (%)', user_name, usr.email;
  END LOOP;
END $$;

-- Verify: show all users and their workspaces
SELECT
  u.full_name,
  au.email,
  w.name as workspace_name,
  w.plan_id,
  m.role,
  (SELECT count(*) FROM public.stages s JOIN public.pipelines p ON s.pipeline_id = p.id WHERE p.workspace_id = w.id) as stages_count
FROM auth.users au
LEFT JOIN public.users u ON u.id = au.id
LEFT JOIN public.memberships m ON m.user_id = au.id
LEFT JOIN public.workspaces w ON w.id = m.workspace_id;
